...
class ControllerExtensionPaymentPayeer extends Controller {
	...
	public function index() {
		...
		$m_key = $this->config->get('payment_payeer_security');
		$data['lang'] = $this->session->data['language'];
		$data['action'] = $this->config->get('payment_payeer_url');
		$data['m_shop'] = $this->config->get('payment_payeer_merchant');
		...
		$this->model_checkout_order->addOrderHistory($data['m_orderid'], $this->config->get('payment_payeer_order_wait_id'));
	...
	public function status() {
		...
		if (isset($request["m_operation_id"]) && isset($request["m_sign"])) {
			...
			$log_file = $this->config->get('payment_payeer_logfile');
			...
			$sign_hash = strtoupper(hash('sha256', implode(":", array(
				...
				$request['m_desc'],
				$request['m_status'],
				$this->config->get('payment_payeer_security')
			))));
			
			$valid_ip = true;
			$sIP = str_replace(' ', '', $this->config->get('payment_payeer_ipfilter'));
				...
				if (!$order) {
					...
					if (!$err) {
						switch ($request['m_status']) {
							case 'success':
								if ($order['order_status_id'] !== $this->config->get('payment_payeer_order_success_id')) {
									$this->model_checkout_order->addOrderHistory($request['m_orderid'], $this->config->get('payment_payeer_order_success_id'));
								}
								break;
								
							default:
								if ($order['order_status_id'] !== $this->config->get('payment_payeer_order_fail_id')) {
									$message .= $this->language->get('text_email_message3') . "\n";
									$this->model_checkout_order->addOrderHistory($request['m_orderid'], $this->config->get('payment_payeer_order_fail_id'));
									$err = true;
								}
								break;
						}
			...
			if ($err) {
				$to = $this->config->get('payment_payeer_email');
				if (!empty($to)) {
